...
class SpatialConv3d(nn.Conv3d):
    ...
    def forward(self, x):
        ...
        if self.reshape_dim == 1:
            ...
            # import pdb;pdb.set_trace()
            input_data = x[:, :, 1:, :].reshape(-1, shape[1], self.token_num, self.token_num, shape[3]).permute(0, 1, 4,
                                                                                                                2, 3)
            x_cls = x[:, :, 0:1, :]
        weight = self.weight
        if self.padding_mode == 'same':
            ...
            if pad_h > 0 or pad_w > 0 or pad_d > 0:
                input_data = F.pad(input_data,
                                   [pad_d // 2, pad_d - pad_d // 2, pad_h // 2, pad_h - pad_h // 2, pad_w // 2,
                                    pad_w - pad_w // 2])
        # import pdb;pdb.set_trace()
        output = F.conv3d(input_data, weight, self.bias, self.stride, self.padding, self.dilation, self.groups).reshape(
            shape[0], shape[1], shape[2], shape[3] - 1)
        return torch.cat([output.permute(0, 1, 3, 2), x_cls], dim=2) if self.reshape_dim == 0 else torch.cat(
            [output, x_cls], dim=2)
...
class SpatialConv2d(nn.Conv2d):
    ...
    def forward(self, x):
        ...
        return torch.cat([output.reshape(shape[0], shape[1], shape[2], shape[3] - 1), x_cls],
                         dim=3) if self.reshape_dim == 1 \
...
class SpatialSelfAttention(nn.Module):
    def __init__(self, inp, oup, kernel_size=3, stride=1, expand_ratio=6, transform_mode='conv2d'):
        ...
        if transform_mode == 'none':
            self.depth_sep_conv = nn.Conv2d(hidden_dim, hidden_dim, (kernel_size, kernel_size), (stride, stride),
            ...
            self.row_conv = nn.Sequential(
                ...
                nn.BatchNorm2d(hidden_dim),
                nn.ReLU6(inplace=True),
                SpatialConv2d(hidden_dim, hidden_dim, kernel_size=(kernel_size[0], 1), reshape_dim=1,
                              groups=hidden_dim),
                nn.BatchNorm2d(hidden_dim),
                nn.ReLU6(inplace=True)
            )
