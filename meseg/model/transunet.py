...
from .inception_transformer_block import InceptionBlock
...
class Encoder(nn.Module):
    def __init__(self, config, vis):
        ...
        for _ in range(config.transformer["num_layers"]):
            ...
            elif config.block == "inception":
                layer = InceptionBlock(
                    dim=config.hidden_size,
                    num_heads=config.transformer["num_heads"],
                    num_path=config.num_path,
                    pixshuf_factor=config.pixshuf_factor,
...
class TransUnet(nn.Module):
    def __init__(self, config, img_size:int=224, vis=False):
